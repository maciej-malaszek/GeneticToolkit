...
namespace GeneticToolkit.Utils.TSPLIB.TSP
{
    ...
    public class Route
    {
        ...
        private static Tuple<double, double> ToGeographical(Vector2F64 v)
        {
            ...
            var min = v.X - deg;
            var latitude = Pi * (deg + 5.0 * min / 3.0) / 180;

            deg = (int) (v.Y + 0.5);
            min = v.Y - deg;
            var longitude = Pi * (deg + 5.0 * min / 3.0) / 180;
        ...
        protected static double GetDistance(EDistanceMetric distanceMetric, Vector2F64 p0, Vector2F64 p1)
        {
            ...
            switch (distanceMetric)
            {
                ...
                case EDistanceMetric.Geographical:
                    ...
                    (var latitude0, var longitude0) = ToGeographical(p0);
                    (var latitude1, var longitude1) = ToGeographical(p1);
                    var q1 = Math.Cos(longitude0 - longitude1);
                    var q2 = Math.Cos(latitude0 - latitude1);
                    var q3 = Math.Cos(latitude0 + latitude1);
