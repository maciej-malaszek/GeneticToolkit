...
namespace GeneticToolkit.Crossovers
{
    ...
    public class MultiPointCrossover : ICrossover
    {
        ...
        {
            var genotypeLength = parents[0].Length;
            var parentIndex = 0;
        ...
        private static byte[] GetMask(int genotypeLength, int[] cutIndexes, int index)
        {
            ...
            for (var i = startCut / GenotypeBase.BitsPerGene + 1;
            ...
            var startIndex = startCut % GenotypeBase.BitsPerGene;
            var endIndex = endCut % GenotypeBase.BitsPerGene;
