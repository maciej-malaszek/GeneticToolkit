...
namespace GeneticToolkit.Crossovers
{
    public class AlternatingEdgeCrossover : ICrossover
    {
        ...
        private short[] GetChildValues(IReadOnlyList<AdjacencyListGenotype> parents, int genotypeSize, int startingParent)
        {
        ...
        {
            AdjacencyListGenotype[] parents = new AdjacencyListGenotype[baseParents.Length]; 
            ...
            for (var j = 0; j < ChildrenCount; j++)
            {
                ...
                ((AdjacencyListGenotype) children[j]).Value = GetChildValues(parents, genotypeSize, j);
            }
            return children;
        }
